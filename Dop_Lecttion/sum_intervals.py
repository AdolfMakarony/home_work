...
def sum_len_intervals(intervals: list):
    sums = []
    for sum_len in intervals:
        if sum_len[0] <= sum_len[-1] and len(sum_len) == 2:
            sums.append(sum_len[-1] - sum_len[0])
        else:
            print('Error arg')
    return sum(sums)
...
def sum_intervals(intervals: list):
    sums2=[]
    for sum_int in intervals:
        if sum_int[0] <= sum_int[-1] and len(sum_int) == 2:
            dev=list(range(sum_int[0], sum_int[-1]))
            # print(dev)
            sums2.append(sum(dev))
        else:
            print('Error arg')
    return sum(sums2)
